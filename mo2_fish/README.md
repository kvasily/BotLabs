...
Open an MP4/MKV and scrub the timeline. Draw an **outer search ROI**, then optionally draw an **inner template crop** inside it. Drawing freezes the displayed frame. Save PNG + ROI crops the native frame, resizes the PNG into game pixels, and patches the corresponding existing YAML fields; Save ROI only preserves the template reference. For `compass`, use the whole widget as the outer box and N as the inner crop. Fish-name/progress roles update `quest_list`/`quest_tooltip`, so their outer box should cover the whole search region. The role menu also includes the four fish-specific hook/bait sources, bait empty/zero states, and logout confirmation. Choose Bassle or Redline Torp when teaching hook/bait slots.

**Recorded at 1440p, play at 4K — set both; Fit is default.** The Video label reads the decoded frame size automatically. Set **Game / monitor capture** to your live resolution (4K, 1440p, 1080p or Custom), choose Fit or Stretch, and click **Apply target / mode**. The cursor readout shows preview → video → game coordinates. For a Steam 2560×1440 recording targeting 3840×2160, a video box `[10,20,100,50]` saves as game ROI `[15,30,150,75]` and a 150×75 PNG. Upscaling uses cubic interpolation; shrinking uses area interpolation. Both inner and outer edges use the same nearest-pixel rounding, and selections smaller than 2×2 game pixels are rejected. This corrects scale; recording compression can still affect match quality.

Fit preserves aspect and centers the source in the game canvas. Aspect differences above 0.01 show a red warning; wider source video produces top/bottom padding, narrower video produces side padding. Padding has no source pixels: clicks in preview bars are ignored, and saved ROIs extending into game padding cannot be drawn back as valid source selections. Stretch explicitly permits different horizontal/vertical scales. Dashed blue outlines show the selected role's saved game ROI mapped back into the preview. Resizing the preview changes only display scaling, never YAML. The decoded ndarray dimensions override container metadata; both are shown when they disagree. Prefer constant-frame-rate recordings for timeline accuracy.

The profile stores `resolution: [W,H]` for live capture, `video_source_resolution: [W,H]` for the last decoded teacher frame, and `video_scale_mode: fit|stretch`. Changing the game target when assets/boxes exist offers **Rescale existing assets**, **Keep existing pixels**, or Cancel. Rescaling transforms ROIs, inventory sources, logout coordinates and compass geometry, writes new PNGs under `templates/rescaled_<id>/`, and retains original images plus `config.before-resolution-<id>.yaml`. Shared/external PNGs are copied, never overwritten. Restore that backup YAML to undo. Keeping existing pixels still expires validation. Nonuniform Stretch of an existing circular compass marks its radius TODO for recalibration. Changing mapping mode alone affects future video crops; it does not alter saved game pixels. Video remains **authoring only**; live `mss` and the overlay use the configured game resolution and `game_origin`.
...
From the `mo2_fish` folder, run `..\.venv\Scripts\python.exe -m unittest discover -s tests -v`. There are 70 tests, including Steam 1440p→4K mapping, real scaled PNG export through the original preflight, resolution migration, and GUI/input wrappers. The executable's `--smoke-test <output-directory>` mode verifies disarmed startup, rendering, bundled media decoding and native overlay properties with no game input. Temporary test media is never saved in user profiles or bundled.
