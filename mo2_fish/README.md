...
Click **FIX** on a failed validation row to open its editor: Teach selects the crop/sound role, Audio device focuses the endpoint, and Profile selects the exact YAML line. Red field/role hints clear when you click or focus the target. Every profile field is listed beside the YAML with a **?** explanation; Teach and Audio controls have matching help buttons. Click **?** or hover for two seconds to read what to enter and how to capture it.

Middle-click the waveform to seek the video; drag the Audio tools splitter down to collapse or up to reopen it.
